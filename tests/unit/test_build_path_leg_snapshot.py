...
SCRIPT_PATH = Path(__file__).resolve().parents[2] / "scripts" / "build_path_leg_snapshot.py"
spec = importlib.util.spec_from_file_location("build_path_leg_snapshot", SCRIPT_PATH)
...
def test_build_path_leg_snapshot_script_writes_latest_json(tmp_path: Path) -> None:
    ...
    json_output_path = tmp_path / "path_leg_snapshot_latest.json"
    ...
    assert payload["summary"]["active_leg_count"] == 3
    assert payload["summary"]["missing_leg_row_count"] == 0
