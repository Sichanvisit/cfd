...
def _runtime_row_with_bridge() -> dict[str, object]:
    row: dict[str, object] = {
        "symbol": "NAS100",
...
def test_bc4_detector_surfaces_state25_context_bridge_weight_review(monkeypatch) -> None:
    monkeypatch.setattr(
        detector_module,
    ...
    payload = detector_module.build_improvement_log_only_detector_snapshot(
        ...
        runtime_status_detail_payload={"latest_signal_by_symbol": {"NAS100": _runtime_row_with_bridge()}},
        ...
        timezone=KST,
        now_ts="2026-04-14T10:10:00+09:00",
    )

    row = payload["candle_weight_detector"]["surfaced_rows"][0]
    preview = row["weight_patch_preview"]

    assert "state25 context bridge weight review" in str(row["summary_ko"]).lower()
    assert preview["bridge_source_lane"] == "STATE25_CONTEXT_BRIDGE_WEIGHT_ONLY_LOG_ONLY"
    assert preview["bridge_weight_requested_count"] >= 1
    assert row["registry_binding_ready"] is True
    issue_ref = next(
        raw
        for raw in payload["feedback_issue_refs"]
        if "weight review" in str(raw.get("summary_ko", "")).lower()
    )
    assert issue_ref["weight_patch_preview"]["bridge_source_lane"] == "STATE25_CONTEXT_BRIDGE_WEIGHT_ONLY_LOG_ONLY"
