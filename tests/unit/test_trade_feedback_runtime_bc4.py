...
def _bc4_issue_ref() -> dict[str, object]:
    runtime_row = {
        "symbol": "NAS100",
    ...
    preview = build_state25_weight_patch_review_candidate_from_context_bridge_v1(runtime_row)
    return {
        "feedback_ref": "D4",
        "detector_key": "candle_weight_detector",
        "detector_label_ko": "candle/weight detector",
        "symbol": "NAS100",
        "summary_ko": "NAS100 state25 context bridge weight review 후보",
...
def test_bc4_proposal_snapshot_surfaces_state25_context_bridge_weight_review_candidates() -> None:
    payload = build_manual_trade_proposal_snapshot(
        _sample_closed_frame(),
        recent_trade_limit=50,
        timezone=KST,
        now_ts="2026-04-14T10:20:00+09:00",
        detector_latest_issue_refs=[_bc4_issue_ref()],
    )

    assert payload["state25_context_bridge_weight_review_count"] == 1
    candidate = payload["state25_context_bridge_weight_review_candidates"][0]
    assert candidate["bridge_source_lane"] == "STATE25_CONTEXT_BRIDGE_WEIGHT_ONLY_LOG_ONLY"
    assert candidate["bridge_weight_requested_count"] >= 1
    assert any(
        "state25 context bridge weight review" in str(line).lower()
        for line in payload["report_lines_ko"]
    )
    assert (
        payload["proposal_envelope"]["evidence_snapshot"]["state25_context_bridge_weight_review_count"]
        == 1
    )


def test_bc4_proposal_snapshot_falls_back_to_detector_snapshot_for_weight_preview() -> None:
    full_issue = _bc4_issue_ref()
    slim_issue = {
        key: value
        for key, value in full_issue.items()
        if key not in {"weight_patch_preview"}
    }
    ...
    payload = build_manual_trade_proposal_snapshot(
        ...
        recent_trade_limit=50,
        timezone=KST,
        now_ts="2026-04-14T10:27:00+09:00",
    ...
    assert payload["state25_context_bridge_weight_review_count"] == 1
    candidate = payload["state25_context_bridge_weight_review_candidates"][0]
    assert candidate["source_feedback_ref"]
    assert candidate["bridge_weight_requested_count"] >= 1
