...
from backend.services.state_polarity_d0_stability_guard import (
    build_state_polarity_d0_stability_report_v1,
...
def test_state_polarity_d0_stability_report_ready_when_dependencies_are_fresh(tmp_path):
    report = build_state_polarity_d0_stability_report_v1(
        ...
        state_structure_dominance_report=_report(tmp_path, "state_structure_dominance"),
        dominance_accuracy_report=_report(tmp_path, "dominance_accuracy"),
        symbol_specific_state_strength_calibration_report=_report(tmp_path, "symbol_specific_state_strength"),
        freshness_threshold_sec=999999,
    )

    summary = report["summary"]
    assert summary["status"] == "READY"
    assert summary["summary_ready_count"] == 7
    assert summary["artifact_ready_count"] == 7
    assert summary["fresh_dependency_count"] == 7


def test_state_polarity_d0_stability_report_hold_when_artifact_is_missing(tmp_path):
...
def test_state_polarity_d0_stability_report_blocked_on_missing_summary_or_upstream_block(tmp_path):
    report = build_state_polarity_d0_stability_report_v1(
        runtime_signal_wiring_audit_report={},
        ca2_r0_stability_report=_report(tmp_path, "ca2_r0_stability"),
        should_have_done_report=_report(tmp_path, "should_have_done", status="BLOCKED"),
        canonical_surface_report=_report(tmp_path, "canonical_surface"),
        state_structure_dominance_report=_report(tmp_path, "state_structure_dominance"),
        dominance_accuracy_report=_report(tmp_path, "dominance_accuracy"),
        symbol_specific_state_strength_calibration_report=_report(tmp_path, "symbol_specific_state_strength"),
...
def test_generate_and_write_state_polarity_d0_stability_report_writes_artifacts(tmp_path):
    report = generate_and_write_state_polarity_d0_stability_report_v1(
        runtime_signal_wiring_audit_report=_report(tmp_path, "runtime_signal_wiring_audit"),
        ca2_r0_stability_report=_report(tmp_path, "ca2_r0_stability"),
        should_have_done_report=_report(tmp_path, "should_have_done"),
        canonical_surface_report=_report(tmp_path, "canonical_surface"),
    ...
    json_path = tmp_path / "state_polarity_d0_stability_latest.json"
    md_path = tmp_path / "state_polarity_d0_stability_latest.md"
