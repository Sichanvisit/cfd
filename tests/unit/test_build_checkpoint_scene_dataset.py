...
SCRIPT_PATH = Path(__file__).resolve().parents[2] / "scripts" / "build_checkpoint_scene_dataset.py"
spec = importlib.util.spec_from_file_location("build_checkpoint_scene_dataset", SCRIPT_PATH)
...
def test_build_checkpoint_scene_dataset_script_writes_scene_csv(tmp_path: Path) -> None:
    resolved_path = tmp_path / "checkpoint_dataset_resolved.csv"
    output_path = tmp_path / "checkpoint_scene_dataset.csv"
    pd.DataFrame(
        [
            {
    ...
    rc = module.main(
        [
            ...
            str(resolved_path),
            "--scene-dataset-output-path",
            str(output_path),
        ]
    )

    assert rc == 0
    scene_dataset = pd.read_csv(output_path, encoding="utf-8-sig")
    assert len(scene_dataset) == 1
    assert scene_dataset.iloc[0]["hindsight_scene_fine_label"] == "breakout_retest_hold"
