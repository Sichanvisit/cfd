...
STATE_POLARITY_D0_STABILITY_GUARD_VERSION = "state_polarity_d0_stability_guard_v1"
STATE_POLARITY_D0_DEFAULT_FRESHNESS_SEC = 900
STATE_POLARITY_D0_DEPENDENCY_KEYS = (
...
def _build_dependency_state(
    ...
    *,
    ...
    summary_freshness_state = _freshness_state(summary_age_seconds, threshold_sec=freshness_threshold_sec)
    ...
    artifact_freshness_state = _freshness_state(artifact_age_seconds, threshold_sec=freshness_threshold_sec)
    ...
    return {
        ...
        "summary_age_seconds": round(float(summary_age_seconds), 1) if summary_age_seconds is not None else None,
        ...
        "artifact_age_seconds": round(float(artifact_age_seconds), 1) if artifact_age_seconds is not None else None,
        "artifact_paths": {
...
def build_state_polarity_d0_stability_report_v1(
    *,
    ...
    state_structure_dominance_report: Mapping[str, Any] | None = None,
    dominance_accuracy_report: Mapping[str, Any] | None = None,
    symbol_specific_state_strength_calibration_report: Mapping[str, Any] | None = None,
    freshness_threshold_sec: int = STATE_POLARITY_D0_DEFAULT_FRESHNESS_SEC,
) -> dict[str, Any]:
    now_dt = _now()
    dependency_reports = {
        ...
        "state_structure_dominance_summary_v1": state_structure_dominance_report,
        "dominance_accuracy_summary_v1": dominance_accuracy_report,
        "symbol_specific_state_strength_calibration_summary_v1": symbol_specific_state_strength_calibration_report,
    }
    dependencies = {
    ...
    for key in STATE_POLARITY_D0_DEPENDENCY_KEYS:
    ...
    if blocked_dependencies:
        ...
        status = "READY"
        reasons = ["decomposition_existing_instrumentation_stable"]

    summary = {
        "generated_at": _now_iso(),
        "status": status,
        "status_reasons": reasons,
        "dependency_count": int(len(STATE_POLARITY_D0_DEPENDENCY_KEYS)),
    ...
    return {
        "contract_version": STATE_POLARITY_D0_STABILITY_GUARD_VERSION,
        "summary": summary,
        "dependency_states_v1": dependencies,
    }


def render_state_polarity_d0_stability_markdown_v1(report: Mapping[str, Any] | None) -> str:
    payload = _mapping(report)
    summary = _mapping(payload.get("summary"))
    dependency_states = {
        key: _mapping(value)
    ...
    lines = [
        "# State Polarity D0 Stability Guard",
        "",
...
def generate_and_write_state_polarity_d0_stability_report_v1(
    *,
    runtime_signal_wiring_audit_report: Mapping[str, Any] | None = None,
    ca2_r0_stability_report: Mapping[str, Any] | None = None,
    should_have_done_report: Mapping[str, Any] | None = None,
    ...
    report = build_state_polarity_d0_stability_report_v1(
        ...
        dominance_accuracy_report=dominance_accuracy_report,
        symbol_specific_state_strength_calibration_report=symbol_specific_state_strength_calibration_report,
        freshness_threshold_sec=freshness_threshold_sec,
    )
    json_path = output_dir / "state_polarity_d0_stability_latest.json"
    md_path = output_dir / "state_polarity_d0_stability_latest.md"
    report["artifact_paths"] = {
        "json_path": str(json_path),
        "markdown_path": str(md_path),
    }
    _write_json(json_path, report)
    _write_text(md_path, render_state_polarity_d0_stability_markdown_v1(report))
    return report
