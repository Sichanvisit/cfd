"""Closed-history enrichment for barrier replay/outcome bridge rows."""
...
BARRIER_SEED_ENRICHMENT_VERSION = "barrier_seed_enrichment_v1"
...
def _resolve_closed_row_index_from_bridge_row(
    ...
    *,
    ...
    outcome = _as_mapping(row.get("barrier_outcome_bridge_v1"))
...
def _barrier_confidence_rank(value: str) -> int:
...
def _build_enrichment_value(rows: Sequence[Mapping[str, Any]] | None = None) -> dict[str, Any]:
    ...
    outcome = _as_mapping(selected.get("barrier_outcome_bridge_v1"))
    label = _to_str(outcome.get("barrier_outcome_label", "")).lower()
    confidence = _to_str(outcome.get("barrier_label_confidence", "")).lower()
    anchor_context = _to_str(outcome.get("barrier_anchor_context", "")).lower()
    anchor_side = _to_str(outcome.get("barrier_anchor_side", "")).upper()
    primary_component = _to_str(outcome.get("barrier_primary_component", "")).lower()
    quality_status = _to_str(outcome.get("bridge_quality_status", "")).lower()
    reason_parts = [
        f"selected_row_key={_to_str(selected.get('row_key', '')) or 'unknown'}",
        f"linked_bridge_rows={len(candidates)}",
    ]
    label_mix = _compact_mix(
        [_to_str(_as_mapping(item.get("barrier_outcome_bridge_v1")).get("barrier_outcome_label", "")).lower() for item in candidates],
        transform=str,
    )
    confidence_mix = _compact_mix(
        [_to_str(_as_mapping(item.get("barrier_outcome_bridge_v1")).get("barrier_label_confidence", "")).lower() for item in candidates],
        transform=str,
    )
    context_mix = _compact_mix(
        [_to_str(_as_mapping(item.get("barrier_outcome_bridge_v1")).get("barrier_anchor_context", "")).lower() for item in candidates],
        transform=str,
    )
    if label_mix:
    ...
    explicit_reason = _to_str(outcome.get("barrier_outcome_reason", "")).lower()
    if explicit_reason:
    ...
    return {
        ...
        "barrier_horizon_bars": _to_int(outcome.get("barrier_horizon_bars", 0), 0),
        "barrier_primary_component": primary_component,
        "barrier_outcome_label": label,
        "barrier_label_confidence": confidence,
        "barrier_bridge_quality_status": quality_status,
        "barrier_outcome_reason": "|".join(reason_parts),
        "barrier_cost_loss_avoided_r": _to_float(outcome.get("barrier_cost_loss_avoided_r", 0.0), 0.0),
        "barrier_cost_profit_missed_r": _to_float(outcome.get("barrier_cost_profit_missed_r", 0.0), 0.0),
        "barrier_cost_wait_value_r": _to_float(outcome.get("barrier_cost_wait_value_r", 0.0), 0.0),
    }


def _existing_enrichment_present(row: pd.Series | Mapping[str, Any]) -> bool:
    return bool(
        _to_str(row.get("barrier_outcome_label", ""))
        or _to_str(row.get("barrier_label_confidence", ""))
        or _to_str(row.get("barrier_primary_component", ""))
    )


def build_barrier_seed_enrichment_plan(
    ...
    *,
    ...
    primary_component_distribution: dict[str, int] = {}
    ...
    for row_index, rows in grouped.items():
        ...
        label = _to_str(enrichment.get("barrier_outcome_label", ""))
        if not label:
            continue
        for target, source in (
            (label_distribution, "barrier_outcome_label"),
            (confidence_distribution, "barrier_label_confidence"),
            (anchor_context_distribution, "barrier_anchor_context"),
            (primary_component_distribution, "barrier_primary_component"),
        ):
    ...
    return {
        "contract_version": BARRIER_SEED_ENRICHMENT_VERSION,
        ...
        "primary_component_distribution": dict(sorted(primary_component_distribution.items())),
        "preview_samples": preview_samples,
    }


def apply_barrier_seed_enrichment(
    frame: pd.DataFrame | None,
    *,
    replay_report: Mapping[str, Any] | None = None,
    overwrite_existing: bool = False,
    ...
    dataset = normalize_trade_df(frame)
    initial_plan = build_barrier_seed_enrichment_plan(
        dataset,
    ...
    for row_index, rows in grouped.items():
        if _existing_enrichment_present(dataset.loc[row_index]) and not overwrite_existing:
            ...
            continue
        enrichment = _build_enrichment_value(rows)
        if not _to_str(enrichment.get("barrier_outcome_label", "")):
...
def load_barrier_outcome_bridge_report(path: str | Path) -> dict[str, Any]:
