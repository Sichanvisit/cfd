...
STATE25_THRESHOLD_PATCH_APPLY_HANDLERS_CONTRACT_VERSION = (
    "state25_threshold_patch_apply_handlers_v0"
)
...
def _render_markdown(payload: Mapping[str, Any] | None) -> str:
    ...
    lines = [
        ...
        f"- rollout_phase: `{_to_text(row.get('rollout_phase'))}`",
        f"- direction: `{_to_text(row.get('threshold_direction'))}`",
        f"- delta_points: `{float(row.get('threshold_delta_points', 0.0) or 0.0):.2f}`",
    ...
    reason_keys = _as_str_list(row.get("threshold_reason_keys"))
    if reason_keys:
...
class State25ThresholdPatchApplyHandlerSet:
    def __init__(
        self,
        *,
    ...
    def register_into(self, apply_executor: ApplyExecutor) -> None:
        apply_executor.register_handler(
            ...
            self.handle_threshold_patch_review,
        )

    def handle_threshold_patch_review(
        self,
        *,
        ...
        proposed_patch = _mapping(review.get("threshold_patch"))
        ...
        bounded_live_enabled = binding_mode in {"bounded_live", "canary"}
        ...
        delta_points = max(
            ...
            int(
                round(
                    _to_float(
                        proposed_patch.get("state25_threshold_log_only_effective_points"),
                        _to_float(
                            proposed_patch.get("state25_threshold_log_only_requested_points"),
                            0.0,
                        ),
                    )
                )
        ...
        direction = _to_text(
            proposed_patch.get("state25_threshold_log_only_direction"),
            "HARDEN",
        ).upper()
        reason_keys = _as_str_list(
            proposed_patch.get("state25_threshold_log_only_reason_keys")
        )
        candidate_id = _to_text(
            review.get("candidate_id"),
            _to_text(existing_state.get("active_candidate_id"), "state25_threshold_patch"),
        )
        applied_at = _to_text(now_ts, _now_iso())
        apply_state = (
            "STATE25_THRESHOLD_PATCH_BOUNDED_LIVE_ACTIVE"
            if bounded_live_enabled
            else "STATE25_THRESHOLD_PATCH_LOG_ONLY_ACTIVE"
        )
        last_event = (
            "apply_state25_threshold_patch_bounded_live"
            if bounded_live_enabled
            else "apply_state25_threshold_patch_log_only"
        )
            ...
            "desired_runtime_patch": {
                **existing_patch,
                ...
                "state25_threshold_log_only_delta_points": (
                    int(delta_points) if not bounded_live_enabled else 0
                ),
                "state25_threshold_log_only_direction": (
                    direction if not bounded_live_enabled else ""
                ),
                "state25_threshold_log_only_reason_keys": (
                    list(reason_keys) if not bounded_live_enabled else []
                ),
                "state25_threshold_log_only_max_adjustment_abs": (
                    int(abs(delta_points)) if not bounded_live_enabled else 0
                ),
                "state25_threshold_bounded_live_enabled": bool(bounded_live_enabled),
                "state25_threshold_bounded_live_delta_points": (
                    int(delta_points) if bounded_live_enabled else 0
                ),
                "state25_threshold_bounded_live_direction": (
                    direction if bounded_live_enabled else "HARDEN"
                ),
                "state25_threshold_bounded_live_reason_keys": (
                    list(reason_keys) if bounded_live_enabled else []
                ),
            },
        }
        ...
        artifact = {
            "contract_version": STATE25_THRESHOLD_PATCH_APPLY_HANDLERS_CONTRACT_VERSION,
            ...
            "threshold_delta_points": float(delta_points),
            "threshold_direction": direction,
            "threshold_reason_keys": list(reason_keys),
            "symbol_scope": symbol_scope,
            "entry_stage_scope": entry_stage_scope,
            "approval_id": _to_text(event_payload.get("approval_id")),
            "scope_key": _to_text(
                event_payload.get("scope_key"),
                _to_text(_mapping(group).get("scope_key")),
            ),
            "active_candidate_state_path": str(self._active_candidate_state_path),
        }
        json_path = (
            self._shadow_auto_dir / "checkpoint_state25_threshold_patch_apply_latest.json"
        )
        md_path = (
            self._shadow_auto_dir / "checkpoint_state25_threshold_patch_apply_latest.md"
        )
        ...
        return {
            "summary": {
                ...
                "recommended_next_action": (
                    "collect_state25_bounded_live_threshold_patch_evidence"
                    if bounded_live_enabled
                    else "collect_state25_log_only_threshold_patch_evidence"
                ),
                "review_type": "STATE25_THRESHOLD_PATCH_REVIEW",
                "candidate_id": candidate_id,
            },
...
def register_default_state25_threshold_patch_apply_handlers(
    apply_executor: ApplyExecutor,
    *,
    active_candidate_state_path: str | Path | None = None,
    shadow_auto_dir: str | Path | None = None,
) -> State25ThresholdPatchApplyHandlerSet:
    handler_set = State25ThresholdPatchApplyHandlerSet(
