...
class Config:
    ...
    TG_CHECK_CHAT_ID = os.getenv("TELEGRAM_CHECK_CHAT_ID", "")
    TG_CHECK_TOPIC_ID = _get_int("TELEGRAM_CHECK_TOPIC_ID", 0)
    TG_REPORT_CHAT_ID = os.getenv("TELEGRAM_REPORT_CHAT_ID", os.getenv("TELEGRAM_PNL_FORUM_CHAT_ID", ""))
    TG_REPORT_TOPIC_ID = _get_int("TELEGRAM_REPORT_TOPIC_ID", 0)
    TG_PNL_FORUM_CHAT_ID = os.getenv("TELEGRAM_PNL_FORUM_CHAT_ID", "")
    TG_PNL_TOPIC_15M_ID = _get_int("TELEGRAM_PNL_TOPIC_15M_ID", 0)
    TG_PNL_TOPIC_1H_ID = _get_int("TELEGRAM_PNL_TOPIC_1H_ID", 0)
    TG_PNL_TOPIC_4H_ID = _get_int("TELEGRAM_PNL_TOPIC_4H_ID", 0)
    TG_PNL_TOPIC_1D_ID = _get_int("TELEGRAM_PNL_TOPIC_1D_ID", 0)
    TG_PNL_TOPIC_1W_ID = _get_int("TELEGRAM_PNL_TOPIC_1W_ID", 0)
    TG_PNL_TOPIC_1M_ID = _get_int("TELEGRAM_PNL_TOPIC_1M_ID", 0)
    TG_ALLOWED_USER_IDS = tuple(
        int(item)
        for item in _get_csv("TELEGRAM_ALLOWED_USER_IDS", "")
        if str(item).lstrip("-").isdigit()
    )
    TG_OPS_ENABLED = _get_bool("TELEGRAM_OPS_ENABLED", True)
    TG_OPS_STATE_PATH = os.getenv("TELEGRAM_OPS_STATE_PATH", r"data\runtime\telegram_ops_state.json")
    TG_OPS_BOOTSTRAP_ENABLED = _get_bool("TELEGRAM_OPS_BOOTSTRAP_ENABLED", True)
    TG_OPS_CALLBACK_POLL_SEC = _get_float("TELEGRAM_OPS_CALLBACK_POLL_SEC", 2.0)
    TG_OPS_PNL_SCAN_SEC = _get_float("TELEGRAM_OPS_PNL_SCAN_SEC", 15.0)
    TG_OPS_CHECK_SCAN_SEC = _get_float("TELEGRAM_OPS_CHECK_SCAN_SEC", 5.0)
    TG_OPS_CHECK_MAX_PENDING = _get_int("TELEGRAM_OPS_CHECK_MAX_PENDING", 5)
    TG_OPS_CHECK_MIN_CONFIDENCE = _get_float("TELEGRAM_OPS_CHECK_MIN_CONFIDENCE", 0.58)
    TG_OPS_LIVE_CHECK_APPROVALS_ENABLED = _get_bool(
        "TELEGRAM_OPS_LIVE_CHECK_APPROVALS_ENABLED",
        False,
    )
    ...
    RUNTIME_REVERSE_ALERT_COOLDOWN_SEC = _get_float("RUNTIME_REVERSE_ALERT_COOLDOWN_SEC", 120.0)
    MAGIC_NUMBER = 9592
    ...
    # [05] AI 진입/청산 보정
    # =========================================
    AI_RUNTIME_ENABLED = _get_bool("AI_RUNTIME_ENABLED", True)
    ...
    # teacher-label exploration entry overlay
    # - keep the main policy intact
    # - only allow narrowly scoped soft-guard bypasses for data collection
    ENTRY_TEACHER_LABEL_EXPLORATION_ENABLED = _get_bool(
        "ENTRY_TEACHER_LABEL_EXPLORATION_ENABLED",
        False,
    )
    ENTRY_TEACHER_LABEL_EXPLORATION_ALLOWED_SYMBOLS = _get_csv(
        "ENTRY_TEACHER_LABEL_EXPLORATION_ALLOWED_SYMBOLS",
        "NAS100,XAUUSD,BTCUSD",
    )
    ENTRY_TEACHER_LABEL_EXPLORATION_ALLOWED_OBSERVE_REASONS = _get_csv(
        "ENTRY_TEACHER_LABEL_EXPLORATION_ALLOWED_OBSERVE_REASONS",
        "lower_rebound_probe_observe,upper_reject_probe_observe,upper_break_fail_confirm,outer_band_reversal_support_required_observe",
    )
    ENTRY_TEACHER_LABEL_EXPLORATION_ALLOWED_SOFT_BLOCKS = _get_csv(
        "ENTRY_TEACHER_LABEL_EXPLORATION_ALLOWED_SOFT_BLOCKS",
        "probe_promotion_gate,probe_not_promoted,forecast_guard,barrier_guard,outer_band_guard,energy_soft_block,execution_soft_blocked,observe_state_wait,consumer_entry_not_ready",
    )
    ENTRY_TEACHER_LABEL_EXPLORATION_REQUIRE_FLAT_POSITION = _get_bool(
        "ENTRY_TEACHER_LABEL_EXPLORATION_REQUIRE_FLAT_POSITION",
        True,
    )
    ENTRY_TEACHER_LABEL_EXPLORATION_MAX_SAME_DIR_COUNT = int(_get_float(
        "ENTRY_TEACHER_LABEL_EXPLORATION_MAX_SAME_DIR_COUNT",
        1,
    ))
    ENTRY_TEACHER_LABEL_EXPLORATION_MIN_SCORE_RATIO = _get_float(
        "ENTRY_TEACHER_LABEL_EXPLORATION_MIN_SCORE_RATIO",
        0.82,
    )
    ENTRY_TEACHER_LABEL_EXPLORATION_MAX_THRESHOLD_GAP = _get_float(
        "ENTRY_TEACHER_LABEL_EXPLORATION_MAX_THRESHOLD_GAP",
        60.0,
    )
    ENTRY_TEACHER_LABEL_EXPLORATION_SIZE_MULTIPLIER = _get_float(
        "ENTRY_TEACHER_LABEL_EXPLORATION_SIZE_MULTIPLIER",
        0.40,
    )
    ENTRY_TEACHER_LABEL_EXPLORATION_SIZE_MULTIPLIER_BY_SYMBOL = {
        "BTCUSD": _get_float(
            "ENTRY_TEACHER_LABEL_EXPLORATION_SIZE_MULTIPLIER_BTCUSD",
            0.30,
        ),
        "NAS100": _get_float(
            "ENTRY_TEACHER_LABEL_EXPLORATION_SIZE_MULTIPLIER_NAS100",
            0.45,
        ),
        "XAUUSD": _get_float(
            "ENTRY_TEACHER_LABEL_EXPLORATION_SIZE_MULTIPLIER_XAUUSD",
            0.40,
        ),
        "DEFAULT": _get_float(
            "ENTRY_TEACHER_LABEL_EXPLORATION_SIZE_MULTIPLIER_DEFAULT",
            ENTRY_TEACHER_LABEL_EXPLORATION_SIZE_MULTIPLIER,
        ),
    }
    ...
    ADVERSE_WAIT_TF_CONFIRM_MAX_SECONDS = _get_float("ADVERSE_WAIT_TF_CONFIRM_MAX_SECONDS", 90.0)
    ADVERSE_WAIT_TF_CONFIRM_WEAK_PEAK_USD = _get_float("ADVERSE_WAIT_TF_CONFIRM_WEAK_PEAK_USD", 0.50)
    ADVERSE_WAIT_TF_CONFIRM_WEAK_PEAK_MAX_SECONDS = _get_float(
        "ADVERSE_WAIT_TF_CONFIRM_WEAK_PEAK_MAX_SECONDS", 45.0
    )
    ...
    ADVERSE_WEAK_PEAK_USD = _get_float("ADVERSE_WEAK_PEAK_USD", 0.25)
    ADVERSE_WEAK_PEAK_MIN_HOLD_SECONDS = _get_float(
    ...
    IMMEDIATE_REVERSE_FLAT_WAIT_SEC = _get_float("IMMEDIATE_REVERSE_FLAT_WAIT_SEC", 1.2)
    IMMEDIATE_REVERSE_PENDING_TTL_SEC = _get_float("IMMEDIATE_REVERSE_PENDING_TTL_SEC", 20.0)
    ...
    ENABLE_EXIT_RUNNER_PRESERVATION = _get_bool("ENABLE_EXIT_RUNNER_PRESERVATION", True)
    EXIT_RUNNER_PRESERVATION_MIN_PEAK_USD = _get_float("EXIT_RUNNER_PRESERVATION_MIN_PEAK_USD", 0.80)
    EXIT_RUNNER_PRESERVATION_MAX_GIVEBACK_USD = _get_float("EXIT_RUNNER_PRESERVATION_MAX_GIVEBACK_USD", 0.90)
    EXIT_RUNNER_PRESERVATION_MAX_GIVEBACK_RATIO = _get_float("EXIT_RUNNER_PRESERVATION_MAX_GIVEBACK_RATIO", 0.55)
    EXIT_RUNNER_PRESERVATION_MIN_FAVORABLE_MOVE_PCT = _get_float("EXIT_RUNNER_PRESERVATION_MIN_FAVORABLE_MOVE_PCT", 0.0008)
    EXIT_RUNNER_PRESERVATION_DEFAULT_HOLD_RATIO = _get_float("EXIT_RUNNER_PRESERVATION_DEFAULT_HOLD_RATIO", 0.92)
    EXIT_RUNNER_PRESERVATION_XAU_HOLD_RATIO = _get_float("EXIT_RUNNER_PRESERVATION_XAU_HOLD_RATIO", 0.82)
    ...
    EXIT_MEANINGFUL_GIVEBACK_PRESSURE_ENABLED = _get_bool(
        "EXIT_MEANINGFUL_GIVEBACK_PRESSURE_ENABLED",
        True,
    )
    EXIT_MEANINGFUL_GIVEBACK_MIN_PEAK_USD = _get_float(
        "EXIT_MEANINGFUL_GIVEBACK_MIN_PEAK_USD",
        1.0,
    )
    EXIT_MEANINGFUL_GIVEBACK_MIN_USD = _get_float(
        "EXIT_MEANINGFUL_GIVEBACK_MIN_USD",
        0.55,
    )
    EXIT_MEANINGFUL_GIVEBACK_RATIO = _get_float(
        "EXIT_MEANINGFUL_GIVEBACK_RATIO",
        0.35,
    )
    EXIT_MEANINGFUL_GIVEBACK_EXIT_BONUS = _get_float(
        "EXIT_MEANINGFUL_GIVEBACK_EXIT_BONUS",
        0.26,
    )
    EXIT_MEANINGFUL_GIVEBACK_HOLD_PENALTY = _get_float(
        "EXIT_MEANINGFUL_GIVEBACK_HOLD_PENALTY",
        0.30,
    )
    EXIT_MEANINGFUL_GIVEBACK_WAIT_PENALTY = _get_float(
        "EXIT_MEANINGFUL_GIVEBACK_WAIT_PENALTY",
        0.18,
    )
    EXIT_COUNTERTREND_TOPDOWN_PRESSURE_ENABLED = _get_bool(
        "EXIT_COUNTERTREND_TOPDOWN_PRESSURE_ENABLED",
        True,
    )
    EXIT_COUNTERTREND_TOPDOWN_MIN_PEAK_USD = _get_float(
        "EXIT_COUNTERTREND_TOPDOWN_MIN_PEAK_USD",
        0.40,
    )
    EXIT_COUNTERTREND_TOPDOWN_MIN_GIVEBACK_USD = _get_float(
        "EXIT_COUNTERTREND_TOPDOWN_MIN_GIVEBACK_USD",
        0.15,
    )
    EXIT_COUNTERTREND_TOPDOWN_EXIT_BONUS = _get_float(
        "EXIT_COUNTERTREND_TOPDOWN_EXIT_BONUS",
        0.18,
    )
    EXIT_COUNTERTREND_TOPDOWN_HOLD_PENALTY = _get_float(
        "EXIT_COUNTERTREND_TOPDOWN_HOLD_PENALTY",
        0.22,
    )
    EXIT_COUNTERTREND_TOPDOWN_WAIT_PENALTY = _get_float(
        "EXIT_COUNTERTREND_TOPDOWN_WAIT_PENALTY",
        0.14,
    )
    EXIT_COUNTERTREND_CONTINUATION_PRESSURE_ENABLED = _get_bool(
        "EXIT_COUNTERTREND_CONTINUATION_PRESSURE_ENABLED",
        True,
    )
    EXIT_COUNTERTREND_CONTINUATION_MIN_PEAK_USD = _get_float(
        "EXIT_COUNTERTREND_CONTINUATION_MIN_PEAK_USD",
        0.80,
    )
    EXIT_COUNTERTREND_CONTINUATION_MIN_GIVEBACK_USD = _get_float(
        "EXIT_COUNTERTREND_CONTINUATION_MIN_GIVEBACK_USD",
        0.50,
    )
    EXIT_COUNTERTREND_CONTINUATION_EXIT_BONUS = _get_float(
        "EXIT_COUNTERTREND_CONTINUATION_EXIT_BONUS",
        0.16,
    )
    EXIT_COUNTERTREND_CONTINUATION_HOLD_PENALTY = _get_float(
        "EXIT_COUNTERTREND_CONTINUATION_HOLD_PENALTY",
        0.18,
    )
    EXIT_COUNTERTREND_CONTINUATION_WAIT_PENALTY = _get_float(
        "EXIT_COUNTERTREND_CONTINUATION_WAIT_PENALTY",
        0.12,
    )
    EXIT_COUNTERTREND_NO_GREEN_FAST_CUT_ENABLED = _get_bool(
        "EXIT_COUNTERTREND_NO_GREEN_FAST_CUT_ENABLED",
        True,
    )
    EXIT_COUNTERTREND_NO_GREEN_MAX_PEAK_USD = _get_float(
        "EXIT_COUNTERTREND_NO_GREEN_MAX_PEAK_USD",
        0.10,
    )
    EXIT_COUNTERTREND_NO_GREEN_PRESSURE_ENABLED = _get_bool(
        "EXIT_COUNTERTREND_NO_GREEN_PRESSURE_ENABLED",
        True,
    )
    EXIT_COUNTERTREND_NO_GREEN_EXIT_BONUS = _get_float(
        "EXIT_COUNTERTREND_NO_GREEN_EXIT_BONUS",
        0.24,
    )
    EXIT_COUNTERTREND_NO_GREEN_HOLD_PENALTY = _get_float(
        "EXIT_COUNTERTREND_NO_GREEN_HOLD_PENALTY",
        0.28,
    )
    EXIT_COUNTERTREND_NO_GREEN_WAIT_PENALTY = _get_float(
    ...
    LOSS_QUALITY_SHORT_TIMEOUT_SECONDS = _get_float("LOSS_QUALITY_SHORT_TIMEOUT_SECONDS", 20.0)
    LOSS_QUALITY_SHORT_TIMEOUT_MAX_DELAY_TICKS = int(os.getenv("LOSS_QUALITY_SHORT_TIMEOUT_MAX_DELAY_TICKS", 3))
    LOSS_QUALITY_SHORT_TIMEOUT_MAX_PEAK_USD = _get_float("LOSS_QUALITY_SHORT_TIMEOUT_MAX_PEAK_USD", 0.10)
    LOSS_QUALITY_SHORT_TIMEOUT_MAX_POST_EXIT_MFE = _get_float("LOSS_QUALITY_SHORT_TIMEOUT_MAX_POST_EXIT_MFE", 0.50)
    ...
    RUNTIME_RECYCLE_MODE = str(os.getenv("RUNTIME_RECYCLE_MODE", "log_only") or "log_only").strip().lower()
    RUNTIME_RECYCLE_INTERVAL_SEC = int(os.getenv("RUNTIME_RECYCLE_INTERVAL_SEC", 3600))
    RUNTIME_RECYCLE_FLAT_GRACE_SEC = int(os.getenv("RUNTIME_RECYCLE_FLAT_GRACE_SEC", 30))
    RUNTIME_RECYCLE_POST_ORDER_GRACE_SEC = int(os.getenv("RUNTIME_RECYCLE_POST_ORDER_GRACE_SEC", 90))
    RUNTIME_RECYCLE_SIGNAL_STALE_SEC = int(os.getenv("RUNTIME_RECYCLE_SIGNAL_STALE_SEC", 900))
    RUNTIME_RECYCLE_DRIFT_MIN_ROWS = int(os.getenv("RUNTIME_RECYCLE_DRIFT_MIN_ROWS", 40))
    RUNTIME_RECYCLE_DRIFT_STAGE_DOMINANCE = _get_float("RUNTIME_RECYCLE_DRIFT_STAGE_DOMINANCE", 0.85)
    RUNTIME_RECYCLE_DRIFT_BLOCK_DOMINANCE = _get_float("RUNTIME_RECYCLE_DRIFT_BLOCK_DOMINANCE", 0.85)
    RUNTIME_RECYCLE_DRIFT_DECISION_DOMINANCE = _get_float("RUNTIME_RECYCLE_DRIFT_DECISION_DOMINANCE", 0.90)
    RUNTIME_RECYCLE_DRIFT_SIGNAL_MIN_COUNT = int(os.getenv("RUNTIME_RECYCLE_DRIFT_SIGNAL_MIN_COUNT", 2))
    STATE25_CANDIDATE_WATCH_APPLY_AI6 = _get_bool("STATE25_CANDIDATE_WATCH_APPLY_AI6", False)
