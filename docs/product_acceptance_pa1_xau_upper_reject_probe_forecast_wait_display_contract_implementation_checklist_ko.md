# Product Acceptance PA1 XAU Upper-Reject Probe Forecast Wait Display Contract Implementation Checklist

작성일: 2026-03-31 (KST)

## Step 0. Family 고정

- [x] target family를 `XAUUSD + upper_reject_probe_observe + forecast_guard + probe_not_promoted + xau_upper_sell_probe`로 고정
- [x] representative backlog row 시간대 확인

## Step 1. Policy contract 추가

- [x] `xau_upper_reject_probe_forecast_wait_as_wait_checks` chart wait relief policy 추가
- [x] `event_kind_hint = WAIT`
- [x] `display_mode = wait_check_repeat`
- [x] `display_reason = xau_upper_reject_probe_forecast_wait_as_wait_checks`

## Step 2. Build path 연결

- [x] `xau_upper_reject_probe_forecast_wait_relief` bool 추가
- [x] `blocked_display_reason = forecast_guard` carry 추가

## Step 3. Resolve path 보호

- [x] repeated cadence suppression 예외 추가
- [x] repeated replay에서도 wait contract 유지 확인
