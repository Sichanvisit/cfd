# Product Acceptance PA1 XAU Upper-Break-Fail Confirm Energy Soft-Block Wait Display Contract Implementation Checklist
...
family를
`WAIT + wait_check_repeat + xau_upper_break_fail_confirm_energy_soft_block_as_wait_checks`
contract로 올린다.
...
- [x] latest PA0에서 target family가 `must_show 3` residue인지 확인
- [x] representative replay에서 build / resolve 모두 hidden blocked 상태인지 확인
- [x] suppress owner가 `xau_upper_sell_repeat_suppressed`인지 확인
...
- mirror confirm family가 WAIT contract로 surface되는가
- repeat suppression이 target family를 다시 죽이지 않는가
- fresh exact row가 없어도 queue turnover로 residue가 빠졌는가
