"""Build latest check/color label formalization artifacts."""
...
from backend.services.check_color_label_formalization import (  # noqa: E402
    build_check_color_label_formalization,
    render_check_color_label_formalization_markdown,
)
...
def _default_breakout_aligned_training_seed_path() -> Path:
...
def _default_json_output_path() -> Path:
    return ROOT / "data" / "analysis" / "shadow_auto" / "check_color_label_formalization_latest.json"


def _default_md_output_path() -> Path:
    return ROOT / "data" / "analysis" / "shadow_auto" / "check_color_label_formalization_latest.md"
...
def main() -> int:
    ...
    parser.add_argument("--breakout-aligned-training-seed-path", default=str(_default_breakout_aligned_training_seed_path()))
    ...
    frame, summary = build_check_color_label_formalization(
        ...
        _load_csv(args.breakout_aligned_training_seed_path),
    )
    markdown = render_check_color_label_formalization_markdown(summary, frame)
