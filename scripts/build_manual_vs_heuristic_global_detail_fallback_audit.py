"""Build a recoverability audit from any entry_decisions detail JSONL archive."""
...
from backend.services.manual_vs_heuristic_global_detail_fallback_audit import (  # noqa: E402
    build_manual_vs_heuristic_global_detail_fallback_audit,
    load_matched_cases,
    render_manual_vs_heuristic_global_detail_fallback_audit_markdown,
)
...
def _default_json_output_path() -> Path:
    return ROOT / "data" / "manual_annotations" / "manual_vs_heuristic_global_detail_fallback_audit_latest.json"


def _default_md_output_path() -> Path:
    return ROOT / "data" / "manual_annotations" / "manual_vs_heuristic_global_detail_fallback_audit_latest.md"


def _default_archive_scan_path() -> Path:
    return ROOT / "data" / "manual_annotations" / "manual_vs_heuristic_archive_scan_latest.csv"
...
def main() -> int:
    ...
    parser.add_argument("--archive-scan-path", default=str(_default_archive_scan_path()))
    ...
    frame, summary = build_manual_vs_heuristic_global_detail_fallback_audit(
        ...
        archive_scan_path=args.archive_scan_path,
    ...
    md_output_path.write_text(render_manual_vs_heuristic_global_detail_fallback_audit_markdown(summary), encoding="utf-8")
