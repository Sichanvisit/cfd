"""Build breakout barrier-drag calibration outputs."""
...
from backend.services.breakout_barrier_drag_calibrator import (  # noqa: E402
    build_breakout_barrier_drag_calibrator,
    render_breakout_barrier_drag_calibrator_markdown,
)


def _default_historical_csv_path() -> Path:
    return ROOT / "data" / "analysis" / "shadow_auto" / "breakout_historical_calibration_bridge_latest.csv"


def _default_runtime_csv_path() -> Path:
    return ROOT / "data" / "analysis" / "shadow_auto" / "breakout_runtime_raw_audit_latest.csv"


def _default_csv_output_path() -> Path:
    return ROOT / "data" / "analysis" / "shadow_auto" / "breakout_barrier_drag_calibrator_latest.csv"


def _default_json_output_path() -> Path:
    return ROOT / "data" / "analysis" / "shadow_auto" / "breakout_barrier_drag_calibrator_latest.json"


def _default_md_output_path() -> Path:
    return ROOT / "data" / "analysis" / "shadow_auto" / "breakout_barrier_drag_calibrator_latest.md"


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--historical-csv-path", default=str(_default_historical_csv_path()))
    parser.add_argument("--runtime-csv-path", default=str(_default_runtime_csv_path()))
    ...
    frame, summary = build_breakout_barrier_drag_calibrator(
        args.historical_csv_path,
        args.runtime_csv_path,
    )
    markdown = render_breakout_barrier_drag_calibrator_markdown(summary, frame)
