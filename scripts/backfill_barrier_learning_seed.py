"""
Apply barrier enrichment to trade_closed_history.csv.

Usage:
  python scripts/backfill_barrier_learning_seed.py
"""
...
from backend.services.barrier_seed_enrichment import (  # noqa: E402
    apply_barrier_seed_enrichment,
    build_barrier_seed_enrichment_plan,
    load_barrier_outcome_bridge_report,
)
...
def main() -> int:
    ...
    replay_report = load_barrier_outcome_bridge_report(replay_report_path)

    if args.dry_run:
        report = build_barrier_seed_enrichment_plan(
            frame,
    ...
    updated_frame, report = apply_barrier_seed_enrichment(
        frame,
        replay_report=replay_report,
        overwrite_existing=bool(args.overwrite_existing),
    )
    backup_path = write_closed_history_backfill(
        ...
        updated_frame,
        backup=True,
        backup_suffix="barrier_enrichment",
    )
