"""
Apply belief enrichment to trade_closed_history.csv.

Usage:
  python scripts/backfill_belief_learning_seed.py
"""
...
from backend.services.belief_seed_enrichment import (  # noqa: E402
    apply_belief_seed_enrichment,
    build_belief_seed_enrichment_plan,
    load_belief_outcome_bridge_report,
)
...
def _default_replay_report_path() -> Path:
    return ROOT / "data" / "analysis" / "belief" / "belief_outcome_bridge_latest.json"
...
def main() -> int:
    ...
    replay_report = load_belief_outcome_bridge_report(replay_report_path)

    if args.dry_run:
        report = build_belief_seed_enrichment_plan(
            frame,
    ...
    updated_frame, report = apply_belief_seed_enrichment(
        frame,
        replay_report=replay_report,
        overwrite_existing=bool(args.overwrite_existing),
    )
    backup_path = write_closed_history_backfill(
        ...
        updated_frame,
        backup=True,
        backup_suffix="belief_enrichment",
    )
