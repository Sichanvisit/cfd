"""
Build the SA4 shadow evaluation layer.

Usage:
  python scripts/build_shadow_auto_evaluation.py
"""
...
from backend.services.shadow_auto_evaluation import (  # noqa: E402
    build_shadow_auto_evaluation,
    load_shadow_auto_evaluation_frame,
    render_shadow_auto_evaluation_markdown,
)
...
def _default_csv_output_path() -> Path:
    return ROOT / "data" / "analysis" / "shadow_auto" / "shadow_evaluation_latest.csv"


def _default_json_output_path() -> Path:
    return ROOT / "data" / "analysis" / "shadow_auto" / "shadow_evaluation_latest.json"


def _default_md_output_path() -> Path:
    return ROOT / "data" / "analysis" / "shadow_auto" / "shadow_evaluation_latest.md"
...
def main() -> int:
    ...
    compare_df = load_shadow_auto_evaluation_frame(compare_path)
    candidates_df = load_shadow_auto_evaluation_frame(candidates_path)
    ...
    md_output_path.write_text(
        render_shadow_auto_evaluation_markdown(summary, evaluation),
        encoding="utf-8",
    )
