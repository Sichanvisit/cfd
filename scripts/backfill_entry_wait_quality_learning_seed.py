"""
Apply entry-side wait-quality enrichment to trade_closed_history.csv.
...
from backend.services.entry_wait_quality_seed_enrichment import (  # noqa: E402
    apply_entry_wait_quality_enrichment,
    build_entry_wait_quality_enrichment_plan,
    load_entry_wait_quality_replay_report,
)
...
def main() -> int:
    ...
    replay_report = load_entry_wait_quality_replay_report(replay_report_path)

    if args.dry_run:
        report = build_entry_wait_quality_enrichment_plan(
            frame,
    ...
    updated_frame, report = apply_entry_wait_quality_enrichment(
        frame,
        replay_report=replay_report,
        overwrite_existing=bool(args.overwrite_existing),
    )
    backup_path = write_closed_history_backfill(
        ...
        updated_frame,
        backup=True,
        backup_suffix="entry_wait_quality_enrichment",
    )
