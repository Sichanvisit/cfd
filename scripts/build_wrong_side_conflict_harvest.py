"""Build latest wrong-side conflict harvest artifacts."""
...
from backend.services.wrong_side_conflict_harvest import (  # noqa: E402
    build_wrong_side_conflict_harvest,
    render_wrong_side_conflict_harvest_markdown,
)
...
def _default_csv_output_path() -> Path:
    return ROOT / "data" / "analysis" / "shadow_auto" / "wrong_side_conflict_harvest_latest.csv"


def _default_json_output_path() -> Path:
    return ROOT / "data" / "analysis" / "shadow_auto" / "wrong_side_conflict_harvest_latest.json"


def _default_md_output_path() -> Path:
    return ROOT / "data" / "analysis" / "shadow_auto" / "wrong_side_conflict_harvest_latest.md"


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--entry-decisions-path", default=str(_default_entry_decisions_path()))
    parser.add_argument("--recent-limit", type=int, default=480)
    ...
    frame, summary = build_wrong_side_conflict_harvest(
        _load_csv(args.entry_decisions_path),
        recent_limit=max(1, int(args.recent_limit)),
    )
    markdown = render_wrong_side_conflict_harvest_markdown(summary, frame)
