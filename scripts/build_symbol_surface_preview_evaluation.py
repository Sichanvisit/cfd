"""Build latest symbol-surface preview evaluation artifacts."""
...
from backend.services.symbol_surface_preview_evaluation import (  # noqa: E402
...
def main() -> int:
    ...
    parser.add_argument("--failure-label-path", default=str(_default_shadow_auto_dir() / "failure_label_harvest_latest.json"))
    parser.add_argument("--distribution-gate-path", default=str(_default_shadow_auto_dir() / "distribution_promotion_gate_baseline_latest.json"))
    parser.add_argument("--market-adapter-path", default=str(_default_shadow_auto_dir() / "market_adapter_layer_latest.json"))
    parser.add_argument("--csv-output-path", default=str(_default_shadow_auto_dir() / "symbol_surface_preview_evaluation_latest.csv"))
    parser.add_argument("--json-output-path", default=str(_default_shadow_auto_dir() / "symbol_surface_preview_evaluation_latest.json"))
    parser.add_argument("--md-output-path", default=str(_default_shadow_auto_dir() / "symbol_surface_preview_evaluation_latest.md"))
    ...
    frame, summary = build_symbol_surface_preview_evaluation(
        ...
        failure_label_harvest_payload=_load_json(args.failure_label_path),
        distribution_promotion_gate_payload=_load_json(args.distribution_gate_path),
